...
const Home = () => {
    ...
    const handleSearchBarValue = (e) => {
        ...
        setNewDummyData(dummyData.filter((item) => (item.location.toLowerCase().includes((val).toLowerCase())) || (item.propertyType.toLowerCase().includes(val.toLowerCase())) && (item.price >= Number(val)))
        )
    }
                    ...
                    <ul className='prope'>
                        ...
                        <li>
                            ...
                            <select className='drop p1' name="propertyType" value={filterValue.propertyType} onChange={handleChange}>
                                {/* <option className='text' value="-1">Select Property Type</option> */}
